function getTemplateFilterLabel(filter, isChineseUi) {
    if (isChineseUi) {
        switch (filter) {
            case 'pack-linked':
                return '已关联能力包';
...
function getSkillPackFilterLabel(filter, isChineseUi) {
    if (isChineseUi) {
        switch (filter) {
            case 'tool-required':
                return '工具依赖';
